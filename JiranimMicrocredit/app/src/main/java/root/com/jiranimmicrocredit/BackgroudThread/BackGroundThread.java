                ...
                if (Integer.parseInt(json.getString(SUCCESS)) > 0) {
                    ...
                    if (response.equalsIgnoreCase(Constants.NEWUSER)) {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            loanApplication.put(DBContract.LoanApplication.KEY_INSTALLMENT, areport.getInt(Constants.PARAM_INSTALMENTAMOUNT));
                            loanApplication.put(DBContract.LoanApplication.KEY_PAYMENTDURATION, areport.getInt(Constants.PARAM_LOAN_PAYMENTDURATION));
                        ...
                        setter.setResponse(response);
                        return response;
                    }else if (response.equalsIgnoreCase(Constants.USERACTION_LOANPAYMENT)) {
                        db.resetTables(DBContract.LoanPaid.LOANPAYMENT_TABLE);
                        jsonArray = json.getJSONArray(DATA);
                        Log.e(TAG, "  " +  jsonArray.toString()+ "  count " + jsonArray.length());
                        JSONObject areport = null;
                        for (int i = 0; i < jsonArray.length(); i++) {
                            areport = jsonArray.getJSONObject(i);
                            Log.e(TAG, "  object number " + i + " " + areport.toString());
                            ContentValues loanPaid = new ContentValues();


                            loanPaid.put(DBContract.LoanPaid._ID, areport.getInt(Constants.PARAM_IDPAYMENT));
                            loanPaid.put(DBContract.LoanPaid.KEY_AMOUNTPAID, areport.getString(Constants.PARAM_AMOUNTPAID));
                            loanPaid.put(DBContract.LoanPaid.KEY_IDLOANAPPLICATION, areport.getInt(Constants.PARAM_ID_LOANAPPLICATION));
                            loanPaid.put(DBContract.LoanPaid.KEY_STATUS, areport.getString(Constants.PARAM_STATUS));
                            loanPaid.put(DBContract.LoanPaid.KEY_TIMEADDED, areport.getString(Constants.PARAM_TIMEADDED));

                            _uri = context.getContentResolver().insert(DBContract.LoanPaid.CONTENT_URI, loanPaid);
    ...
    protected void onPostExecute(final String result){
        ...
        if(setter.getResponse()!=""||setter.getResponse()!=null) {
            if (result.equalsIgnoreCase(Constants.ACTION_BUSINESS)) {
                ...
                }else if (setter.getResponse().equalsIgnoreCase(Constants.PARAM_ALLEXISTS)){
                    Toast.makeText(context,Constants.MSG_ALL,Toast.LENGTH_SHORT).show();
                }else if (setter.getResponse().equalsIgnoreCase(Constants.PARAM_EMAILEXISTS)){
                    Toast.makeText(context,Constants.MSG_EMAIL,Toast.LENGTH_SHORT).show();
                }else if (setter.getResponse().equalsIgnoreCase(Constants.PARAM_PHONEEXISTS)){
                    Toast.makeText(context,Constants.MSG_PHONE,Toast.LENGTH_SHORT).show();
                } else if (setter.getResponse().equalsIgnoreCase(Constants.PARAM_IDEXISTS)){
                    Toast.makeText(context,Constants.MSG_IDNUMBER,Toast.LENGTH_SHORT).show();
                }else{
